...
router.patch('/users/:id', (req, res) => {
  ...
  const newMaxServers = maxServers ? Math.max(1, parseInt(maxServers, 10)) : user.max_servers;
  const newMaxRam = maxRamMb ? Math.max(1024, parseInt(maxRamMb, 10)) : user.max_ram_mb;
  ...
  `).run(req.session.user.id, `Admin modified User #${targetUserId} (${user.name}): role=${newRole}, max_servers=${newMaxServers}`);

  return res.json({
    success: true,
    message: `User #${targetUserId} updated successfully.`,
...
router.patch('/settings', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO activity_logs (user_id, action, details, created_at)
    VALUES (?, 'admin_settings_update', 'Platform configuration modified by administrator', datetime('now'))
